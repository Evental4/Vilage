...
const int cont_btn = 14;
//кн очитка
const int btn_delte = cont_btn -7;
//кн сохранение экрана
...
int main()
{
    ...
    btn[4] = {640,30,"прочие струк","прочие струк"};
    ...
    btn[12] = {1100,650,"Выход",""};
    btn[13] = {550,350,"Начать",""};
    ...
    //инц картин меню
     ...
     cont_pic =readFromDir("Pictures/прочие струк/",menupic,cont_pic);
     cont_pic =readFromDir("Pictures/деревья/",menupic,cont_pic);
     cont_pic =readFromDir("Pictures/дорога/",menupic,cont_pic);
    ...
    for(int i=0; i<cont_pic; i++)
{
   ...
   if(menupic[i].category == "рабочие зд" )
    {
        menupic[i].w_scr =  menupic[i].w/2 ;
        menupic[i].h_scr =  menupic[i].h/2 ;
    }
   if(menupic[i].category == "жилые зд" )
    {
        menupic[i].w_scr =  menupic[i].w/2;
        menupic[i].h_scr =  menupic[i].h/2;
    }
   if(menupic[i].category == "другиие струк" )
    {
        menupic[i].w_scr = menupic[i].w/2 ;
        menupic[i].h_scr = menupic[i].h/2 ;
    }
   if(menupic[i].category == "прочие струк" )
    {
        menupic[i].w_scr =  menupic[i].w/2;
        menupic[i].h_scr =  menupic[i].h/2;
    }
   if(menupic[i].category == "деревья" )
    {
        menupic[i].w_scr = menupic[i].w/2;
        menupic[i].h_scr = menupic[i].h/2;
    }
   if(menupic[i].category == "дорога" )
    {
        menupic[i].w_scr = menupic[i].w/1;
        menupic[i].h_scr = menupic[i].h/1;
    }
    ...
    while(!btn[btn_exit].click())
{
    ...
    if (page ==  page_redactor )
    {
        ...
        for (int i=0;i<n_sent_pic; i++)
         ...
         //установка видимости кар меню по категореи
    ...
    else if (page == page_help)
    {
        ...
        txDrawText(0,200, 1200 , 600,
                   ...
                   "Выход из програмы на кнопку'Выход'");
        btn[btn_help].draw();
        if(btn[btn_help].click())
        {
              page = page_redactor;
